...
class Modified_Interval_Halving:
    ...
    def plot_data_intervals(self):
        ...
        plt.title('{}: SAGE Ratio = {}, Frac_Noise = {}'.format(self.__title, \
                    np.round(self.scaled_avg_global_err()/self.__noise_err, 4), np.round(self.frac_noise(), 4)))
    ...
    def plot_data_pred_intervals(self):
        ...
        plt.legend(custom_lines, ['Steady State (S)', 'Transition (T)', 'Noise (N)'])
        plt.title('{}: Noise Error = {}, SAGE = {}'.format(self.__title, np.round(self.__noise_err, 4), \
        np.round(self.scaled_avg_global_err(), 4)))
        plt.xlabel(self.__xlabel)
        plt.ylabel(self.__ylabel)
        # plt.show()  
        return
    ...
    def frac_noise(self):
        ...
        for inter_indices, inter_type in zip(self.__intervals, self.__interval_types):
            if inter_type == 'Noise (N)':
                noise_len += (inter_indices[1] - inter_indices[0] + 1)
    ...
    def train_model(self, data): # data should be passed as 1d numpy array
        ...
        return len(self.__intervals) # Returns the number of intervals obtained by the Interval Halving Algorithm
